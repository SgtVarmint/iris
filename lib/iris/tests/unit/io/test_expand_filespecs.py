...
class TestExpandFilespecs:
    ...
    def test_absolute_path(self):
        result = iio.expand_filespecs([str(self.tmpdir / "*")])
        expected = [str(self.tmpdir / fname) for fname in self.fnames]
        assert result == expected

    def test_double_slash(self):
        product = iio.expand_filespecs([str("//" / self.tmpdir / "*")])
        predicted = [str(self.tmpdir / fname) for fname in self.fnames]
    ...
    def test_relative_path(self):
        ...
        try:
            ...
            item_in = [str(self.tmpdir / fname) for fname in self.fnames]
    ...
    def test_return_order(self):
        ...
        expected = [str(self.tmpdir / fname) for fname in ["a.foo", "b.txt"]]
    ...
    def test_no_files_found(self):
        ...
        with pytest.raises(IOError, match=msg):
            iio.expand_filespecs([str(self.tmpdir / "no_exist.txt")])
    ...
    def test_files_and_none(self):
        ...
        with pytest.raises(IOError, match=re.escape(emsg)):
            iio.expand_filespecs(
                [
                    str(self.tmpdir / "does_not_exist.txt"),
                    str(self.tmpdir / "*"),
                ]
            )

    def test_false_bool_absolute(self):
        msg = str(self.tmpdir / "no_exist.txt")
    ...
    def test_false_bool_relative(self):
        ...
        try:
            ...
            item_in = [str(self.tmpdir / "no_exist.txt")]
            assert item_out == item_in
        finally:
            os.chdir(cwd)
