...
.. z_reference:: iris
   :tags: topic_load_save;topic_data_model;topic_customisation;topic_slice_combine
...
* uris:
    ...
    standard library function :func:`pathlib.Path(filepath).expanduser` and
...
def sample_data_path(*path_to_join):
    ...
    if iris_sample_data is not None:
        target = Path(iris_sample_data.path) / target
    else:
        raise ImportError(
